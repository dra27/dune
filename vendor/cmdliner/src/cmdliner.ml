(*---------------------------------------------------------------------------
  Copyright (c) 2011 Daniel C. Bünzli. All rights reserved. Distributed under
...
module Term = struct
  ...
  let err_multi_cmd_def name (a, _) (a', _) =
    Cmdliner_base.err_multi_def ~kind:"command" name Cmdliner_info.term_doc a
      a'
  ...
  let add_stdopts ei =
    ...
    let vargs, vers =
      match Cmdliner_info.(term_version @@ eval_main ei) with
  ...
  let do_help help_ppf err_ppf ei fmt cmd =
    ...
    let _, _, ei =
      add_stdopts ei
      (* may not be the originally eval'd term *)
    in
  ...
  let do_result help_ppf err_ppf ei = function
    ...
    | Error res -> (
      match res with
      | `Std_help fmt ->
        Cmdliner_docgen.pp_man err_ppf fmt help_ppf ei;
        `Help
      | `Std_version ->
      ...
      | `Parse err ->
        ...
        `Error `Parse
      | `Help (fmt, cmd) ->
        do_help help_ppf err_ppf ei fmt cmd;
        `Help
      | `Exn (e, bt) ->
        Cmdliner_msg.pp_backtrace err_ppf ei e bt;
        `Error `Exn
      | `Error (usage, err) ->
        if usage then
          Cmdliner_msg.pp_err_usage err_ppf ei ~err
  ...
  let eval ?help:(help_ppf = Format.std_formatter)
      ?err:(err_ppf = Format.err_formatter) ?(catch = true)
      ?(env = env_default) ?(argv = Sys.argv) ((al, f), ti) =
  ...
  let choose_term main choices = function
    | [] -> Ok (main, [])
    | maybe :: args' as args -> (
      if String.length maybe > 1 && maybe.[0] = '-' then
        Ok (main, args)
      else
        let index =
  ...
  let eval_choice ?help:(help_ppf = Format.std_formatter)
      ?err:(err_ppf = Format.err_formatter) ?(catch = true)
      ?(env = env_default) ?(argv = Sys.argv) main choices =
